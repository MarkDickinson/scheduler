...
sub JOB_status_display() {
	...
	if ($x == 0) { next; }
        ...
        $failed = 0;
	# MID: The repeat jobs use up too many lines of display
	# when monitoring multiple systems, so suppress those
	# here. NOTE: Still display them if they are not in a 
	# scheduled wait state.
	$testjobname = substr( $jobname, 0, 6 );
	if ($testjobname eq "SYS-R0") {
           if ($status eq "SCHEDUL") {
              next;
	   }
	}
	   ...
	   $failed = 2; # stop the action butten being displayed
	   print "\n";  # and need this
        }
        elsif ($failed == 0) {
        ...
        if ($test ne "SCHEDULER") {
           if ($failed != 2) {
              ...
              $urlname = 'html_job_sched_action.pl?html_job_status_full.pl='.${jobname}."=".${ipaddr}."=".${schedhostname};
              print ' ';
    ...
    if ($testfail eq 'Connection refused') {
       print "<span style=\"background-color: #FF0000\">${schedhostname} **** JOB SCHEDULER IS NOT AVAILABLE **** Check with support staff</span><br><br><br>";
    }
    elsif ($jobcount == 0) {
       print "<span style=\"background-color: #FF0000\">${schedhostname} ${dataline}</span><br>\n";
    }
...
print &PrintHeader;
&scheduler_common_header( "html_job_status_full_no_repeatjobs.pl", 120000 );
...
# Find out how many systems we are looking at, then look at them
