<%
   ...
   out.println("<H2>Job database entries</H2>");
   out.println( "<b><pre> Server                Job Name                       Next runtime             Description</pre></b>" );
   out.println( "<table border=\"1\" cellpadding=\"5\"><tr><td><pre>" );

   cmdLine.append( API_CMD_JOB_LISTALL );  // outside the loop you wally !.
   ...
   for (int i = 0; i < numHosts; i++ ) {
      ...
      if (zz != 0) {     // error, use socketpool.gethostname as for some reason hostname is empty in this block
                         // bugger, still blank
         out.println( "<span style=\"background-color: #FF0000\">" + socketPool.getHostName(i) + ":**Error** " + getCmdrespcodeAsString(zz) + "</span><br>" );
         out.println( "<br>");
      }
   } // for numhosts
   ...
   for (int i = 0; i < numHosts; i++ ) {
      ...
      if ( (dataLine.equals("100")) || (dataLine.equals("+++")) ) {     // response display text to display or non API data
         ...
         while ( (dataLine != "") && (dataLine.length() >= 31)) {
            ...
            while (dataLine.length() < 98) { dataLine = dataLine + " "; }
            // add buttons
            out.println( hostname2 + ": " +
                         dataLine  + "<a href=\"" + request.getScheme() +
                         "://" + request.getServerName() + ":8080" + request.getRequestURI() +
                         "?pagename=jobdisplaymenu&jobname=" + jobName +
                         "&server=" + hostname + "&action=display\"><img src=\"buttons/Button_Actions_Blue.jpg\"></a> " );
            dataLine = socketPool.getResponseLine( hostname, session.getId() );
         } // while dataline != ""
      } // if response code = 100
      else {
         out.print( "<span style=\"background-color: #FF0000\">" + hostname + ":**Error** " );
         if (dataLine.equals("101")) {     // raw data returned (binary, not display data)
            ...
            out.println( hostname + ":There is no longer a data buffer associated with this server" );
         }
         else {
   ...
   out.println( "End of job listall display" );
%>
