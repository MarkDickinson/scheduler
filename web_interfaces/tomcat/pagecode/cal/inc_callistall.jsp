<%
   ...
   String dataLine;
   String calName, calYear, calType;
   StringBuffer cmdLine = new StringBuffer();

   hostname2 = ""; // stop may not be initialised errors
   out.println("<H2>Calendar database entries</H2>");
   out.println( "<b><pre> Server              : Year CalTyp  Calendar-Name                           Description</pre></b>" );
   out.println( "<table border=\"1\" cellpadding=\"5\"><tr><td><pre>" );

   numHosts = socketPool.getHostMax();
   for (int i = 0; i < numHosts; i++ ) {
      hostname = socketPool.getHostName(i);
	  cmdLine.append( API_CMD_CAL_LISTALL );
   ...
   for (int i = 0; i < numHosts; i++ ) {
      ...
      if ( (dataLine.equals("100")) || (dataLine.equals("+++")) ) {     // response display text to display or non API data
         ...
         while ( (dataLine != "") && (dataLine.length() >= 33)) {
            calName = dataLine.substring( 13, 33 );
            int firstSpace = calName.indexOf(" ");
            calName = calName.substring( 0, firstSpace );
            calYear = dataLine.substring( 0, 4 );
            calType = dataLine.substring( 5, 8 );
            // Make the line a fixed length so all the colours and buttons line up.
            while (dataLine.length() < 70) { dataLine = dataLine + " "; }
            // add buttons
            out.println( hostname2 + ": " + dataLine +
                         "<a href=\"" + request.getScheme() +
                         "://" + request.getServerName() + ":8080" + request.getRequestURI() +
                         "?pagename=caldisplay&jobname=" + calName + "&action=display" +
                         "&server=" + hostname + "&calyear=" + calYear + "&caltype=" + calType + "\">" +
                         "[SHOW DETAILS]</a> " +
                         "<a href=\"" + request.getScheme() +
                         "://" + request.getServerName() + ":8080" + request.getRequestURI() +
                         "?pagename=calaction&jobname=" + calName + "&action=delete" +
                         "&server=" + hostname + "&calyear=" + calYear + "&caltype=" + calType + "\">" +
                         "<img src=\"buttons/delete_orange.jpg\"></a> " );
            dataLine = socketPool.getResponseLine( hostname, session.getId() );
         } // while dataline != ""
   ...
   out.println( "<br><br>End of calendar listall display" );
%>
