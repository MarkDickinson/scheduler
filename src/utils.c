...
int UTILS_legal_date_yyyymmdd( char * datestr ) {
		char smallbuf[21], *ptr;
		time_t passed_times_stamp;
		time_t timestamp_now;
		int days_left;

		if (pSCHEDULER_CONFIG_FLAGS.debug_level.utils >= DEBUG_LEVEL_PROC) {
				myprintf( "DEBUG: Enetered UTILS_legal_date_yyyymmdd with '%s'\n", datestr );
		}

		if (strlen(datestr) != 8) {
				if (pSCHEDULER_CONFIG_FLAGS.debug_level.utils >= DEBUG_LEVEL_PROC) {
						myprintf( "DEBUG: Leaving UTILS_legal_date_yyyymmdd, date to short\n" );
				}
				return( 0 );
		}

		strncpy( smallbuf, datestr, 8 );
		ptr = (char *)&smallbuf;
		ptr = ptr + 8;
		*ptr = '\0';
		strcat( smallbuf, " 01:01:01" );
		passed_times_stamp = UTILS_make_timestamp( (char *)&smallbuf );
		if (passed_times_stamp < 1) {
				if (pSCHEDULER_CONFIG_FLAGS.debug_level.utils >= DEBUG_LEVEL_PROC) {
						myprintf( "DEBUG: Leaving UTILS_legal_date_yyyymmdd, date is illegal\n" );
				}
				return( 0 );   /* not a legal date */
		}

		timestamp_now = UTILS_timenow_timestamp();  /* this overwrites passed_time_stamp */
		if (timestamp_now > passed_times_stamp) {
				if (pSCHEDULER_CONFIG_FLAGS.debug_level.utils >= DEBUG_LEVEL_PROC) {
						myprintf( "DEBUG: Leaving UTILS_legal_date_yyyymmdd, date is before todays date\n" );
				}
				return( 0 ); /* its before todays date */
		}

		days_left = (int) ( (passed_times_stamp - timestamp_now) / (60 * 60 * 24) );
		if (days_left > 0) {
		    if (days_left < 90) {
		       myprintf( "WARN: ZW003-License for this product expires in %d days\n", days_left );
		    }
		    if (days_left < 10) {
		       myprintf( "WARN: ZW004-License for this product expires in %d days\n", days_left );
		       myprintf( "WARN: ZW004-License for this product expires in %d days\n", days_left );
		       myprintf( "WARN: ZW004-License for this product expires in %d days\n", days_left );
		       myprintf( "WARN: ZW004-License for this product expires in %d days\n", days_left );
		    }
		...
		if (pSCHEDULER_CONFIG_FLAGS.debug_level.utils >= DEBUG_LEVEL_PROC) {
				myprintf( "DEBUG: Leaving UTILS_legal_date_yyyymmdd, date is OK\n" );
		}

        return( 1 );
} /* UTILS_legal_date_yyyymmdd */
